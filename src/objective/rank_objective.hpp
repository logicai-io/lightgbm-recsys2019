...
#pragma omp parallel for schedule(guided)
    ...
    /*!
* \brief Objective function for Lambdrank with MRR
*/
    class LambdarankMRR : public ObjectiveFunction {
    public:
        explicit LambdarankMRR(const Config &config) {
            sigmoid_ = static_cast<double>(config.sigmoid);
            label_gain_ = config.label_gain;
            // initialize DCG calculator
            DCGCalculator::DefaultLabelGain(&label_gain_);
        ...
        explicit LambdarankMRR(const std::vector<std::string> &) {
        }
...
#pragma omp parallel for schedule(guided)
        ...
        inline void GetGradientsForOneQuery(const double *score,
            ...
            for (data_size_t j = 0; j < i; ++j) {
                ...
                double delta_pair_MRR = (((1.0f / (static_cast<double>(j + 1.0f))) -
...
//        }
    ...
    class LambdarankMRR2 : public ObjectiveFunction {
    public:
        explicit LambdarankMRR2(const Config &config) {
            sigmoid_ = static_cast<double>(config.sigmoid);
            label_gain_ = config.label_gain;
            // initialize DCG calculator
            DCGCalculator::DefaultLabelGain(&label_gain_);
        ...
        explicit LambdarankMRR2(const std::vector<std::string> &) {
        }
...
#pragma omp parallel for schedule(guided)
        ...
        inline void GetGradientsForOneQuery(const double *score,
            ...
            // printf("best score=%.4f best index %d\n", best_score, sorted_idx[0]);
            // for (data_size_t i = 0; i < cnt; ++i) {
            //     printf("%d %d %.4f %.4f\n", i, sorted_idx[i], label[i], score[i]);
            // }
        }
    ...
    class LambdarankMRR4 : public ObjectiveFunction {
    public:
        explicit LambdarankMRR4(const Config &config) {
            sigmoid_ = static_cast<double>(config.sigmoid);
            label_gain_ = config.label_gain;
            // initialize DCG calculator
            MRRCalculator::DefaultLabelGain(&label_gain_);
        ...
        explicit LambdarankMRR4(const std::vector<std::string> &) {
        }
